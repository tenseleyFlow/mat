...
HDRS = src/compat.h src/config.h src/config_generated.h src/err.h \
       ...
       src/range.h src/linesrc.h src/rangeprint.h src/frame.h src/encoding.h src/ansi.h src/syntax.h src/highlight.h lib/paige/include/paige.h
...
OBJS = build/main.o build/cli.o build/err.o build/iobuf.o \
       ...
       build/range.o build/linesrc.o build/rangeprint.o build/frame.o build/encoding.o build/ansi.o build/syntax.o build/highlight.o $(PAIGE_OBJS)
...
build/linesrc.o: src/linesrc.c $(HDRS)
	@mkdir -p build
...
build/frame.o: src/frame.c $(HDRS)
	@mkdir -p build
	$(CC) $(CFLAGS) -c src/frame.c -o build/frame.o

build/encoding.o: src/encoding.c $(HDRS)
	@mkdir -p build
	$(CC) $(CFLAGS) -c src/encoding.c -o build/encoding.o

build/ansi.o: src/ansi.c $(HDRS)
	@mkdir -p build
	$(CC) $(CFLAGS) -c src/ansi.c -o build/ansi.o

build/syntax.o: src/syntax.c $(HDRS)
	@mkdir -p build
	$(CC) $(CFLAGS) -c src/syntax.c -o build/syntax.o

build/highlight.o: src/highlight.c $(HDRS)
	@mkdir -p build
	$(CC) $(CFLAGS) -c src/highlight.c -o build/highlight.o
...
asan:
	$(CC) $(CFLAGS) -fsanitize=address,undefined -g -Ilib/paige/src -o mat-asan \
	    ...
	    src/range.c src/linesrc.c src/rangeprint.c src/frame.c src/encoding.c src/ansi.c src/syntax.c src/highlight.c \
